...
data class MenuShortcut(
    ...
    companion object {
        ...
        private fun displayKeyLabel(key: Key) = when (key) {
            ...
            Key.Eight -> "8"
            Key.Nine -> "9"
            Key.Enter, Key.NumPadEnter -> if (OsType.isMacOS) "↵" else "Enter"
            Key.Backspace -> "⌫"
            Key.Delete -> if (OsType.isMacOS) "⌦" else "Del"
            Key.Spacebar -> if (OsType.isMacOS) "␣" else "Space"
            Key.Tab -> if (OsType.isMacOS) "⇥" else "Tab"
            Key.Escape -> "Esc"
            Key.Comma -> ","
            Key.DirectionUp -> "Up"
